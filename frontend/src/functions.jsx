...
const updateBackendProducts = async (data) => {
  try {
    await fetch(buildApiUrl("/api/setProducts"), {
      method: "POST",
...
export const fetchProducts = async (setProducts) => {
  const timeout = setTimeout(() => {
    console.warn("Backend tardando, intento directo desde Firestore...");
  ...
  try {
    ...
    if (!response.ok) {
      console.warn("Backend devolvio error, uso Firestore...");
...
export const addFilter = async (key, value) => {
  try {
    await fetch(buildApiUrl(`/api/filter/${key}/${value}`));
  } catch (error) {
...
export const getFilters = async () => {
  try {
    const response = await fetch(buildApiUrl("/api/getFilters"));
    const data = await response.json();
    return data;
  } catch (error) {
...
export const getFilteredProducts = async (setFilteredProducts) => {
  try {
    const response = await fetch(buildApiUrl("/api/getFilteredProducts"));
    const data = await response.json();
...
export const resetFilters = async () => {
  try {
    const response = await fetch(buildApiUrl("/api/resetFilters"));
    const data = await response.json();
    return data;
  } catch (error) {
...
export const sortProducts = async (sort) => {
  try {
    const response = await fetch(buildApiUrl(`/api/sortBy/${sort}`));
    const data = await response.json();
    return data;
  } catch (error) {
